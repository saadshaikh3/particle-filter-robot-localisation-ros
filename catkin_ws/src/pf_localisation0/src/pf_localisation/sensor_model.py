...
import math
import pf_localisation.laser_trace as laser_trace
...
class SensorModel:
    ...
    def set_laser_scan_parameters(self, num_readings, scan_range_max,
        ...
        :Args:
            ...
            | scan_range_max (float): Max range scanner can read
            | scan_length (int): The number of readings in a complete scan
            | scan_angle_min (float): The min. angle of the scanner
            | scan_angle_max (float): The max. angle of the scanner
        """
        ...
        self.reading_points = [
            (i, scan_angle_min + ((scan_angle_max - scan_angle_min) * (float(i) / scan_length)))
    ...
    def set_map(self, occupancy_map):
        ...
        :Args:
            | occupancy_map (nav_msgs.msg.OccupancyGrid): the map to use
        """
        ...
        self.map_origin_x = (
            occupancy_map.info.origin.position.x +
            (self.map_width / 2.0) * self.map_resolution
        )
        self.map_origin_y = (
            occupancy_map.info.origin.position.y +
    ...
    def calc_map_range(self, ox, oy, oa):
        ...
        :Args:
            | ox (float): X location of observation
            | oy (float): Y location of observation
            | oa (float): Bearing (from North, in degrees) of the reading
        :Returns:
            | (float) Range (in m) expected to be observed by the laser
        """
        #print('1111111111111111111111111111111111111111111111')
        #print(laser_trace)
        #print(type(laser_trace))
        #print('type(self.map_data):',type(self.map_data))
        #print('len(self.map_data):',len(self.map_data))
        #print('type(self.map_data[0]):', type(self.map_data[0]))
        #print('self.map_data:',self.map_data)
        #print('dir(laser_trace):',dir(laser_trace))
        r = laser_trace.map_calc_range(
            ox, oy, oa,
            self.map_width, self.map_height,
            self.map_origin_x, self.map_origin_y,
            self.map_resolution, self.scan_range_max,
            self.map_data
        )
        return min(r, self.scan_range_max)
    ...
    def get_weight(self, scan, pose):
        ...
        :Returns:
            | (float) Likelihood weighting for this particle, given the map
              and laser readings
        """
        ...
        for i, obs_bearing in self.reading_points:
            ...
            p += pz**3  # Cube probability: reduce low-probability particles
    ...
    def predict(self, obs_range, map_range):
        ...
        :Args:
            | obs_range (float): Observed range (i.e., from actual laser)
            | map_range (float): Predicted range (i.e., from map)
        :Returns:
            | (float) Probability [0,1] that we would observe the obs_range
        ...
        pz += self.z_hit * math.exp(-(z**2) / (2 * self.sigma_hit**2))

        # Part 2: short reading from unexpected obstacle (e.g., a person)
        if z < 0:
            pz += self.z_short * self.lambda_short * math.exp(-self.lambda_short * obs_range)
        ...
        if obs_range < self.scan_range_max:
            pz += self.z_rand / self.scan_range_max

        return max(0.0, min(pz, 1.0))
